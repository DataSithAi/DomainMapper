...
def read_config(cfg_file):
    try:
        ...
        dns_server_indices = (
            list(map(int, config.get("dnsserver", "").split()))
        ...
        if cfginfo in ["yes", "y"]:
            ...
            if filetype in ["mikrotik", ""]:
                ...
                print(
                    f"{Style.BRIGHT}'comment=' в Mikrotik firewall:{Style.RESET_ALL} {'вЫключен' if mk_comment == 'off' else 'включен'}"
                )
            print(
                f"{Style.BRIGHT}Сохранить результат в файл:{Style.RESET_ALL} {filename}"
            )
            print(
        ...
        try:
            ...
            for ip_address in ips:
                if (
                    ...
                    or ip_address in resolver.nameservers
                ):
                    null_ips_count[0] += 1
                    ...
                    print(
                        f"{Fore.BLUE}{domain} IP-адрес: {ip_address} - {dns_server_name}{Style.RESET_ALL}"
                    )
            return filtered_ips
        except Exception as e:  # Ловим все ошибки чтобы код не прервался
            print(
                f"{Fore.RED}Не удалось получить IP-адрес: {domain} - {dns_server_name}{Style.RESET_ALL}"
            )
            return []
        ...
        while True:
            ...
            if selection.strip():
                ...
                if "0" in selections:
                    ...
                    services = [
                        list(urls.keys())[int(sel) - 1]
                        for sel in selections
                        if sel.isdigit() and 1 <= int(sel) <= len(urls)
                    ]
...
def check_dns_servers(dns_servers, dns_server_indices):
    ...
    dns_server_options = [("Системный DNS", system_dns_servers)] + list(
        dns_servers.items()
    )
    ...
    if dns_server_indices:
        if (
            0 in dns_server_indices
        ):  # Если указано 0, выбираем все доступные DNS серверы
            selected_dns_servers = dns_server_options
        else:
            for idx in dns_server_indices:
                if (
                    1 <= idx <= len(dns_server_options)
                ):  # Корректируем индекс на 1 меньше, чтобы соответствовать списку
...
def comment(selected_service):
    return ",".join(
        ["".join(word.title() for word in s.split()) for s in selected_service]
    )
...
def group_ips_in_subnets(filename, subnet):
    try:
        ...
        if subnet in ["24", "16"]:
            process_ips(subnet)
            print(
                f"{Style.BRIGHT}IP-адреса агрегированы до /{subnet} подсети{Style.RESET_ALL}"
            )
            ...
            ips -= {
                ip for group in octet_groups.values() if len(group) > 1 for ip in group
            }
            ...
            print(
                f"{Style.BRIGHT}IP-адреса агрегированы до масок /24 и /32{Style.RESET_ALL}"
            )
...
def process_file_format(
    ...
    net_mask = (
        ...
        if subnet == "mix"
            ...
            "255.255.0.0"
            if subnet == "16"
            else "255.255.255.0" if subnet == "24" else "255.255.255.255"
        )
    )
...
Ваш выбор: """
    ...
    formatters = {
        ...
        + (f' comment="{comment(selected_service)}"' if mk_comment != "off" else "")
        + f" address={ip}/{subnet}",
    ...
    if subnet == "mix":
        if filetype.lower() == "win":  # Обработка для win
            ...
            mix_formatter = lambda ip: (
                f"{ip.strip()} 255.255.255.0"
                if ip.endswith(".0")
                else f"{ip.strip()} 255.255.255.255"
            )
        else:  # Обработка для остальных форматов
            mix_formatter = lambda ip: (
                f"{ip.strip()}/24" if ip.endswith(".0") else f"{ip.strip()}/32"
            )
        ...
        formatters.update(
            {
                ...
                "mikrotik": lambda ip: f"/ip/firewall/address-list add list={mk_list_name}"
                + (
                    f' comment="{comment(selected_service)}"'
                    if mk_comment != "off"
                    else ""
                )
                + f" address={mix_formatter(ip)}",
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="DNS resolver script with custom config file."
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    print(
        f"{Style.BRIGHT}Использовались DNS сервера:{Style.RESET_ALL} "
        + ", ".join(
            [f'{pair[0]} ({", ".join(pair[1])})' for pair in selected_dns_servers]
        )
    )
    print(
        f"{Style.BRIGHT}Всего обработано DNS имен:{Style.RESET_ALL} {total_domains_processed[0]}"
    )
    if include_cloudflare:
        print(
            f"{Style.BRIGHT}Исключено IP-адресов Cloudflare:{Style.RESET_ALL} {cloudflare_ips_count[0]}"
        )
    print(
        f"{Style.BRIGHT}Исключено IP-адресов 'заглушек':{Style.RESET_ALL} {null_ips_count[0]}"
    )
    print(
        f"{Style.BRIGHT}Разрешено IP-адресов из DNS имен:{Style.RESET_ALL} {len(unique_ips_all_services)}"
    )
